...
/**
 * StreamDemo — 基于 DeepAgent 的流式输出与多轮对话示例，使用自研文件 Checkpointer 持久化会话。
 *
 * <h3>核心特性</h3>
 * <ul>
 *   <li><b>流式输出</b> — 实时消费 Agent 的流式输出</li>
 *   <li><b>多轮对话</b> — 同一 Session 内多轮交互，Agent 记住历史上下文</li>
 *   <li><b>文件持久化</b> — 通过 {@code RunnerConfig} 配置自研 {@code FileCheckpointer}，
 *        会话状态自动持久化到本地文件，无需手工序列化</li>
 *   <li><b>会话恢复</b> — 使用相同 sessionId 创建会话时自动从文件恢复状态</li>
 * </ul>
 *
 * <h3>使用方式</h3>
 * <pre>{@code
 *   # 交互式多轮对话
 *   java -Dfile.checkpointer.dir=./file_checkpoint_data \
 *        myexample.stream.StreamDemo2
 *
 *   # 单次问答
 *   java -Dfile.checkpointer.dir=./file_checkpoint_data \
 *        myexample.stream.StreamDemo2 --query "计算 3+5"
 *
 *   # 恢复历史会话
 *   java -Dfile.checkpointer.dir=./file_checkpoint_data \
 *        myexample.stream.StreamDemo2 --resume &lt;sessionId&gt;
 * }</pre>
 *
 * <h3>交互命令</h3>
 * <pre>
 ...
 * </pre>
 */
public final class CheckPointerFile {

    private static final Path WORKSPACE_ROOT = Path.of(".").toAbsolutePath().normalize();

    private static volatile boolean fileCheckpointerInitialized;
    ...
    private static AgentSessionApi ensureSession(ConversationState state) {
        ...
        if (oldSession != null) {
            ...
            System.out.println("[INFO] 创建新会话: " + state.sessionId
                    + " (文件持久化: " + (hasFileCheckpointer() ? "ON" : "OFF") + ")");
        }
        ...
        switch (action) {
            ...
            case ":status" -> {
                if (state.sessionId == null) {
                    ...
                    System.out.println("文件持久化: " + (hasFileCheckpointer() ? "已启用" : "未启用（内存模式）"));
                }
            }
            case ":help" -> {
                System.out.println("""
                        ...
                        文件持久化: -Dfile.checkpointer.dir=./file_checkpoint_data""");
            }
    ...
    // =========================================================================
    // File Checkpointer 初始化（自研文件存储）
    // =========================================================================

    /**
     * 通过 {@link RunnerConfig} 配置自研的 FileCheckpointer。
     *
     * <p>JVM 参数:
     * <ul>
     *   <li>{@code -Dfile.checkpointer.dir=./file_checkpoint_data} — 持久化目录</li>
     * </ul>
     *
     * <p>配置后，框架会在每次 Agent 执行前后自动持久化/恢复 Session 状态，
     * 业务代码无需做任何手工序列化。
     */
    private static synchronized void configureFileCheckpointer() {
        if (fileCheckpointerInitialized) {
            return;
        }
        String dir = System.getProperty("file.checkpointer.dir", "./file_checkpoint_data").trim();

        Map<String, Object> fileConf = new LinkedHashMap<>();
        fileConf.put("dir", dir);

        Map<String, Object> checkpointerConfig = new LinkedHashMap<>();
        checkpointerConfig.put("type", "file");
        checkpointerConfig.put("conf", fileConf);

        try {
            CheckpointerFactory.register("file", FileCheckpointer.provider());
            ...
            Runner.start();
            System.out.println("[INFO] 文件 Checkpointer 已启用: " + dir);
        } catch (Exception e) {
            System.err.println("[WARN] 文件 Checkpointer 初始化失败: " + e.getMessage());
            System.err.println("[WARN] 回退到内存模式");
            e.printStackTrace();
        }
        fileCheckpointerInitialized = true;
    }

    private static boolean hasFileCheckpointer() {
        return fileCheckpointerInitialized;
    }
    ...
    private static DeepAgent buildAgent() {
        ...
        Map<String, String> llmConfig = ApiConfigLoader.load();
