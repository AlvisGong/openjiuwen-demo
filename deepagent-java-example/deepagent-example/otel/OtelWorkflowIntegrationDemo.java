...
public class OtelWorkflowIntegrationDemo {
    ...
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        if (jaegerAvailable) {
            LOG.info("View traces in Jaeger UI: http://localhost:16686 (service: my_agent_service1)");
        }
    }

    /**
     * Check whether the OTLP/HTTP collector (the actual span export endpoint) is reachable.
     * <p>Probe the configured OTLP HTTP port (4318), not the Jaeger UI (16686):
     * the UI may be up while the OTLP receiver is not, which would otherwise cause
     * noisy "Failed to export spans" errors during the demo.
     *
     * @return true if localhost:4318 responds with any HTTP status (even 404/405)
     */
    private static boolean isOtlpCollectorAvailable() {
        try {
            ...
            // Any HTTP response means the OTLP HTTP port is listening; only connection
            // failures (refused/timeout) indicate the collector is down.
            return code > 0;
        } catch (IOException e) {
            LOG.debug("OTLP collector not available on localhost:4318: {}", e.getMessage());
            return false;
        }
    }
}
