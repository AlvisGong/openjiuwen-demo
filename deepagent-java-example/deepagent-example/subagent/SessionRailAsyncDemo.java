...
/**
 * SessionRailAsyncDemo — 基于 DeepAgent + SessionRail 的异步（流式）多子 Agent 协作样例。
 *
 * <p>与 {@code SubagentAsyncDemo}（SubagentRail，提供单个 {@code task_tool}）对照，本示例改用
 * <b>SessionRail</b>：当 {@code enableAsyncSubagent=true} 时由 {@link HarnessFactory} 自动注入，
 * 为父 Agent 提供一组会话管理工具。
 *
 * <h3>SessionRail 提供的工具</h3>
 * <ul>
 *   <li><b>sessions_spawn</b> — 派发子任务给指定类型子 Agent；入参
 *       {@code subagent_type} / {@code task_description} / {@code parent_session_id}，
 *       返回 {@code {task_id, sub_session_id, result}}，并在 SessionToolkit 中登记任务状态</li>
 *   <li><b>sessions_list</b> — 列出当前所有子任务及其运行状态</li>
 *   <li><b>sessions_cancel</b> — 按 {@code task_id} 取消子任务</li>
 * </ul>
 * 注意：{@code sessions_spawn} 底层仍调用 {@code TaskTool.delegate}（同步执行子 Agent 的
 * {@code invoke}）；SessionRail 的价值在于<b>会话/任务生命周期管理</b>（任务登记、状态跟踪、
 * 列举、取消），而非真正非阻塞的子 Agent 委派。
 *
 * <h3>为什么这是“异步”</h3>
 * <p>“异步”体现在父 Agent 的执行通道：{@link DeepAgent#stream} 在 {@code enableTaskLoop=true}
 * 时启动守护线程（{@code "deep-agent-stream-<sessionId>"}）跑 {@code runTaskLoop}，立即返回
 * {@code session.streamIterator()}。父 Agent 在后台线程执行（含 sessions_spawn 委派子 Agent），
 * 调用方线程通过迭代器渐进消费输出，端到端非阻塞。
 *
 * <h3>异步协作链路</h3>
 * <pre>
 ...
 *                     └─ 调用 sessions_spawn(subagent_type, task_description, parent_session_id)
 *                          └─ SessionRail.spawn ─> TaskTool.delegate（同步，跑在守护线程里）
 *                               └─ 子 Agent.invoke ─> 完成，结果回传，SessionToolkit 登记
 *                     └─ (可选) sessions_list 查看任务状态 / sessions_cancel 取消任务
 ...
 * </ul>
 * 默认查询同时触发两个子 Agent，演示 SessionRail 的 spawn/list 与流式汇总。
 *
 * <h3>运行方式</h3>
 * <pre>{@code
 *   # 默认查询（计算 + 翻译，触发两个子 Agent，流式输出）
 *   java myexample.subagent.SessionRailAsyncDemo
 *
 *   # 自定义查询
 *   java myexample.subagent.SessionRailAsyncDemo --query "计算 100 / 4 的结果"
 *
 *   # 指定 apiconfig.json 路径
 *   java -Dopenjiuwen.example.config=examples/apiconfig.json myexample.subagent.SessionRailAsyncDemo
 * }</pre>
 *
 * <p><b>注意：</b>子 Agent 通过 {@code TaskTool.delegate} 走 {@code invoke} 路径执行，
 * 因此每个 {@link SubAgentConfig} 都必须设置 {@code isTaskLoopEnabled(true)}。
 */
public final class SessionRailAsyncDemo {

    private static final Path WORKSPACE_ROOT = Path.of(".").toAbsolutePath().normalize();
    private static final String AGENT_ID = "orchestrator_agent_session";
    private static final String DEFAULT_SESSION_ID = "session-rail-async-demo";

    private SessionRailAsyncDemo() {
    }
    ...
    public static void main(String[] args) {
        try {
            ...
            // 2. 构建带子 Agent 的父 DeepAgent（enableAsyncSubagent=true → SessionRail 自动注入）
            DeepAgent agent = buildAgent();

            // 3. 解析查询
            String query = resolveQuery(args);

            System.out.println("========== SessionRail Async(Stream) Demo ==========");
            System.out.println("[INFO] 父 Agent: " + AGENT_ID + " (enableTaskLoop=true, 流式后台执行)");
            System.out.println("[INFO] SessionRail 由 HarnessFactory 自动注入 (enableAsyncSubagent=true)");
            System.out.println("[INFO] 会话工具: sessions_spawn / sessions_list / sessions_cancel");
     ...
     * </ul>
     * 子 Agent 委派（sessions_spawn 等）的调用与返回会以 tool_result 形式出现在流中。
     */
    ...
    private static DeepAgent buildAgent() {
        ...
        // --- 父 Agent 系统提示：引导其用 sessions_spawn 委派子 Agent ---
        String systemPrompt = """
                你是任务编排 Agent（orchestrator）。你本身不直接做算术或翻译，
                而是通过会话管理工具把子任务派发给合适的子 Agent：
                  - math_analyst：算术运算
                  - translator：中英翻译

                可用工具：
                  - sessions_spawn(subagent_type, task_description, parent_session_id)
                      派发子任务，返回 {task_id, sub_session_id, result}。
                  - sessions_list()
                      列出所有子任务及运行状态，可在派发后查看进度。
                  - sessions_cancel(task_id)
                      按需取消尚未完成的子任务。

                工作流程：
                1. 拆解用户请求，识别哪些部分应交给哪个子 Agent；
                2. 对每个子任务调用 sessions_spawn，传入正确的 subagent_type 与清晰、自包含的
                   task_description，parent_session_id 可填当前会话 id；
                3. （可选）调用 sessions_list 确认各任务状态；
                4. 收集各 sessions_spawn 返回的 result；
                5. 汇总后用中文给出最终答复，简要说明各子 Agent 的贡献。
                """;

        AgentCard card = AgentCard.builder()
                .id(AGENT_ID)
                .name(AGENT_ID)
                .description("编排 Agent — 通过 SessionRail 管理并派发子 Agent 任务（异步流式）")
                .build();
        ...
        DeepAgentConfig config = DeepAgentConfig.builder()
                ...
                .enableAsyncSubagent(true)     // 关键：true → HarnessFactory 注入 SessionRail（而非 SubagentRail）
